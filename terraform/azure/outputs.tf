...
output "load_balancer_ip" {
  description = "Public IP address for the Load Balancer (static) or message to check after deployment (dynamic)"
  value       = var.enable_nginx_ingress && var.use_static_ip ? azurerm_public_ip.lb[0].ip_address : (var.enable_nginx_ingress ? "Dynamic IP - run: kubectl get svc -n ingress-nginx ingress-nginx-controller" : null)
}
