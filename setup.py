...
class GCPDeploymentConfig:
    ...
    def __init__(self):
        ...
        # Note: cloudsql_password is auto-generated by Terraform (like AWS RDS), not stored in config
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # cloudsql_password is auto-generated by Terraform, not in config
            'n8n_namespace': self.n8n_namespace,
            'n8n_host': self.n8n_host,
            'n8n_protocol': self.n8n_protocol,
...
class FileUpdater:
    ...
    def create_terraform_tfvars_gcp(self, config: GCPDeploymentConfig):
        """Create terraform.tfvars for GCP GKE infrastructure deployment"""
        tfvars_file = self.base_dir / "terraform" / "gcp" / "terraform.tfvars"
        ...
        lines = [
            ...
            "# GKE Cluster",
            f'cluster_name      = "{config.cluster_name}"',
            f'node_machine_type = "{config.node_machine_type}"',
            f'node_count        = {config.node_count}',
            "",
            "# Network",
            f'vpc_name    = "{config.vpc_name}"',
            f'subnet_name = "{config.subnet_name}"',
            "",
            "# Application",
            f'n8n_namespace      = "{config.n8n_namespace}"',
            f'n8n_encryption_key = "{config.n8n_encryption_key}"',
        ...
        if config.database_type == "cloudsql":
            lines.extend([
                ...
                f'cloudsql_tier              = "{config.cloudsql_tier}"',
                f'cloudsql_database_version  = "POSTGRES_15"',
                f'cloudsql_availability_type = "ZONAL"',
                f'cloudsql_disk_size         = 10',
                f'cloudsql_username          = "n8n"',
                f'cloudsql_password          = "CHANGE_ME_SECURELY"  # IMPORTANT: Change this!',
            ])
        ...
        if config.enable_basic_auth:
            lines.extend([
                ...
                "# Basic Authentication",
                f'enable_basic_auth  = {str(config.enable_basic_auth).lower()}',
                f'basic_auth_username = "{config.basic_auth_username}"',
                f'basic_auth_password = "{config.basic_auth_password}"',
            ])
        else:
            lines.extend([
                "",
        ...
        content = "\n".join(lines) + "\n"
        tfvars_file.write_text(content)
        print(f"{Colors.OKGREEN}✓ Created terraform/gcp/terraform.tfvars{Colors.ENDC}")
...
def load_existing_configuration(script_dir: Path, cloud_provider: str = "aws"):
    ...
    for raw_line in tfvars_path.read_text().splitlines():
        ...
        # Note: cloudsql_password is auto-generated by Terraform, not read from tfvars
...
def main():
    ...
    parser = argparse.ArgumentParser(description='N8N Multi-Cloud Deployment Setup (AWS EKS / Azure AKS / GCP GKE)')
    parser.add_argument('--cloud-provider', type=str, choices=['aws', 'azure', 'gcp'],
                       help='Cloud provider to use (aws, azure, or gcp). If not specified, you will be prompted to choose.')
...
''')
    ...
    if not cloud_provider:
        ...
        prompt = ConfigurationPrompt()
        choice = prompt.prompt_choice("Which cloud provider would you like to use?", ["AWS", "Azure", "GCP"], default=0)
        if choice == "AWS":
            cloud_provider = "aws"
        elif choice == "Azure":
    ...
    try:
        ...
        db_config = {
            ...
            'cloudsql_password': outputs.get('cloudsql_password'),  # Now from Terraform outputs (auto-generated)
        }
